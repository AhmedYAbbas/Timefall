...
namespace Timefall
{
		...
		: m_MSDFData(new MSDFData())
	{
		...
		if (!font)
		{
			TF_CORE_ERROR("Failed to load font: {0}", fileString);
			return;
		}
		...
		TF_CORE_INFO("Loaded {0} glyphs from font (out of {1})", glyphsLoaded, charset.size());
		...
		TF_CORE_ASSERT(remaining == 0, "Failed to pack all glyphs into the atlas");
