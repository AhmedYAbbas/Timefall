...
namespace Timefall
{
	...
	static void ComputeTangents(std::vector<MeshVertex>& verts, const std::vector<uint32_t>& indices)
	{
		...
		for (MeshVertex& v : verts)
		{
			...
			v.Tangent = glm::length(t) > 1e-6f ? glm::normalize(t) : glm::vec3(1.0f, 0.0f, 0.0f);
			v.Bitangent = glm::cross(n, v.Tangent) * (glm::dot(glm::cross(n, v.Tangent), v.Bitangent) < 0.0f ? -1.0f : 1.0f);
		}
	}
