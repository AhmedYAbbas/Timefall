...
namespace Timefall
{
		...
		unsigned source, unsigned type, unsigned id, unsigned severity, int length, const char* message, const void* userParam)
	{
		switch (severity)
		{
			case GL_DEBUG_SEVERITY_HIGH: TF_CORE_CRITICAL(message); return;
			case GL_DEBUG_SEVERITY_MEDIUM: TF_CORE_ERROR(message); return;
			case GL_DEBUG_SEVERITY_LOW: TF_CORE_WARN(message); return;
			case GL_DEBUG_SEVERITY_NOTIFICATION: TF_CORE_TRACE(message); return;
		}
	...
	void OpenGLRendererAPI::Init()
	{
        ...
        glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS); // filter across cube faces; low-res IBL mips seam without it
    }
	...
	void OpenGLRendererAPI::DrawIndexed(const Ref<VertexArray>& vertexArray, uint32_t indexCount, uint32_t indexOffset, uint32_t baseVertex)
	{
		TF_PROFILE_FUNCTION();

		vertexArray->Bind();
		uint32_t count = indexCount ? indexCount : vertexArray->GetIndexBuffer()->GetCount();
		glDrawElementsBaseVertex(GL_TRIANGLES, count, GL_UNSIGNED_INT, (void*)(uintptr_t)(indexOffset * sizeof(uint32_t)), baseVertex);
	}
