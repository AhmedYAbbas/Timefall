...
namespace Timefall
{
	...
	static bool KeyIndexValid(KeyCode key)
	{
		return (int)key >= 0 && (int)key < s_KeyCount;
	}
	static bool MouseIndexValid(MouseCode button)
	{
		return (int)button >= 0 && (int)button < s_MouseButtonCount;
	}

	static void TrackKey(KeyCode key)
	{
		if (KeyIndexValid(key))
			s_TrackedKeys.insert((int)key);
	}
	...
	{
		TrackKey(key);
		return KeyIndexValid(key) && s_CurrentKeys[(int)key];
	}
	bool Input::IsKeyPressedThisFrame(KeyCode key)
	{
		TrackKey(key);
		return KeyIndexValid(key) && s_CurrentKeys[(int)key] && !s_PreviousKeys[(int)key];
	}
	bool Input::IsKeyReleasedThisFrame(KeyCode key)
	{
		TrackKey(key);
		return KeyIndexValid(key) && !s_CurrentKeys[(int)key] && s_PreviousKeys[(int)key];
	}
	...
	{
		return MouseIndexValid(button) && s_CurrentMouseButtons[(int)button];
	}
	bool Input::IsMouseButtonPressedThisFrame(MouseCode button)
	{
		return MouseIndexValid(button) && s_CurrentMouseButtons[(int)button] && !s_PreviousMouseButtons[(int)button];
	}
	bool Input::IsMouseButtonReleasedThisFrame(MouseCode button)
	{
		return MouseIndexValid(button) && !s_CurrentMouseButtons[(int)button] && s_PreviousMouseButtons[(int)button];
	}

	void Input::SetViewportMousePosition(const glm::vec2& position)
	{
		s_ViewportMousePosition = position;
	}
	glm::vec2 Input::GetViewportMousePosition()
	{
		return s_ViewportMousePosition;
	}
}
